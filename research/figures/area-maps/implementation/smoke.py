...
try:
 ...
 cfg=yaml.safe_load((S/'area_rollout.yaml').read_text());cfg.update(robots=['aerial05','aerial08'],dataset=str(W/'reference'),output_root=str(W),experiment_name='120-second accumulated area-map smoke; unchanged coverage odometry; no ground truth')
 cfg['dpgo']['ros_domain_id']=220;cfg['backend']['read_paths']=[native.__file__]
 ...
 watched.update([B/'install/ellipselio/lib/libellipselio_mapping.so',B/'launcher/ellipselio_mapping_mt',ROOT/'ellipselio/CMakeLists.txt',ROOT/'FAST-LIVO2-ROS2/scripts/ellipselio_live_rerun.py',Path(native.__file__),ROOT/'FAST-LIVO2-ROS2/scripts/run_ellipselio.py',ROOT/'FAST-LIVO2-ROS2/include/research_export.h',ROOT/'FAST-LIVO2-ROS2/src/research_export.cpp'])
 ...
 for robot,trial in trials.items():
  ...
  assert np.isfinite(pose).all() and np.all(np.diff(stamp)>0) and speed<20
  state.setdefault('frontend_gate',{})[robot]=gap<1
  state['diagnostic']=not all(state['frontend_gate'].values());save(W/'status.json',state)
  save(W/f'{robot}-quality.json',dict(native_poses=len(updates),finite_chronological=True,max_successful_update_gap_s=gap,max_speed_m_s=speed,smoke_passed=gap<1,full_sequence=False,ground_truth_used=False))
  output=W/f'prepared-{robot}'
  call([sys.executable,'-m','s3e_pipeline.recent_submaps','--source',Path(trial)/'frontend/area_maps','--output',output,'--config',W/'config.yaml'],W/f'{robot}-prepare.log',300)
