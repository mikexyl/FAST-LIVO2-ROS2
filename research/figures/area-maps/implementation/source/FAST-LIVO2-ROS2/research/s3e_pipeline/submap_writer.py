...
def consume(stream, output):
    ...
    with (output/'index.jsonl').open('x') as index:
        while True:
            ...
            if row['schema_version'] not in (1, 3, 4, 5) or any(r['submap_id'] == row['submap_id'] for r in rows):
            ...
            if row['schema_version'] == 4 and row.get('strategy') != 'coverage':
                raise ValueError('Schema 4 requires the coverage strategy')
            if row['schema_version'] == 5 and row.get('strategy') != 'area':
                raise ValueError('Schema 5 requires accumulated area maps')
            arrays = {}
            ...
            if row['schema_version'] == 4:
                count = row['coverage_cell_count']
                if not 0 <= count <= row['geometry_count']:
                    raise ValueError('Invalid coverage cell count')
                arrays['coverage_cells'] = np.frombuffer(exact(stream, count*3*4), dtype='<i4').reshape(-1, 3)
            if row['schema_version'] == 5:
                for name,key in [('point_ids','geometry_count'),('point_scan_ids','geometry_count'),('ellipsoid_point_ids','ellipsoid_count')]:
                    arrays[name]=np.frombuffer(exact(stream,row[key]*4),dtype='<i4')
                sys.path.insert(0,str(Path(__file__).resolve().parents[1]))
                from s3e_pipeline.area_maps import validate_area
                validate_area(row,arrays)
