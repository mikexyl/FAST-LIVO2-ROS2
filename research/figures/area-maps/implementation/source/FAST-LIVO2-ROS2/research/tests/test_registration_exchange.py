...
def network(tmp_path):
    robots = ['Alpha', 'Bob', 'Carol']; workers = {}; queue = []
...
def test_pcm_selection_unique_ownership_and_delayed_evidence(tmp_path):
    ...
    assert [len(m['pairs']) for m in manifests.values()] == [1, 1, 0]
    ...
    assert [len(m['clouds']) for m in manifests.values()] == [2, 2, 0]
