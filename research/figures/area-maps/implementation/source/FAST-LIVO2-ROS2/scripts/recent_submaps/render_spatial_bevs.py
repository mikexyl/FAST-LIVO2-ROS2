...
try:
    for robot in cfg['robots']:
        ...
        for key in keys:
            ...
            area=key.get('strategy')=='area'
            payload = Path(trials[robot])/('frontend/area_maps' if area else 'frontend/submaps')/key['payload']
            ...
            if area:
                ...
                points,sampling=render_area_surface(e[:,:3],e[:,3:6],basis,features['ground'],key['area_radius_m'])
            else:
                if sampler is None:sampler=SurfaceSampler()
            ...
            maps.append(dict(robot=robot, key=index, submap_id=key['submap_id'],
                ...
                strategy=key.get('strategy','spatial'),occupied_area_m2=key.get('occupied_area_m2'),new_area_m2=key.get('new_area_m2'),
                area_radius_m=key.get('area_radius_m'),map_points=key['geometry_count'],
...
fig, axes = plt.subplots((len(cfg['robots'])+1)//2, 2, figsize=(11, 5*((len(cfg['robots'])+1)//2)), squeeze=False, layout='constrained', facecolor='#f3f5f8')
for ax in axes.flat: ax.axis('off')
for ax, robot in zip(axes.flat, cfg['robots']):
    robot_maps = [m for m in maps if m['robot'] == robot]
    item = robot_maps[min(8, len(robot_maps)-1)]
    im = cv2.imread(str(OUT/item['image']), cv2.IMREAD_GRAYSCALE)
    ax.imshow(im, cmap='gray', vmin=0, vmax=255, interpolation='nearest')
    timing=f'at {item["end_s"]:.0f} s' if item['strategy']=='area' else f'{item["start_s"]:.0f}–{item["end_s"]:.0f} s'
    ax.set_title(f'{robot.replace("aerial", "Aerial ")} · submap {item["submap_id"]}\n'
                 f'{timing} · {item["features"]} ORB features', fontsize=13, pad=9)
    ax.axis('off')
...
strategy=maps[0]['strategy']
fig.suptitle(f'GRACO aerial · {strategy} submaps · gravity-horizontal BEVs', fontsize=20)
...
<p>Completed spatial submaps: 40 m horizontal radius, 20 m overlap, and a 120 s age guard. Gravity-horizontal orthographic projection uses the native filter gravity saved at each anchor pose. Durations vary with flight motion. No ground truth was used. These are the native grayscale density images reconstructed from the experiment’s saved ellipsoids. Every image reproduces the cached ORB keypoints and descriptors exactly.</p>
...
if strategy=='coverage':
    html=html.replace('Completed spatial submaps: 40 m horizontal radius, 20 m overlap, and a 120 s age guard.',
        'Completed observed-coverage submaps: occupied gravity-grid cells, new observed area and measured shared area drive handovers. Motion and age are secondary guards.')
if strategy=='area':
    html=html.replace('Completed spatial submaps: 40 m horizontal radius, 20 m overlap, and a 120 s age guard.',
        f'Accumulated area maps: all stored native map points within {maps[0]["area_radius_m"]:g} m horizontally of the drone at each snapshot, at any height and from any earlier scan. Odometry retains its separate recent map.')
    html=html.replace('Durations vary with flight motion.','Times identify snapshots; they are not scan-membership windows.')
    html=html.replace('${m.start_s.toFixed(0)}–${m.end_s.toFixed(0)} s','snapshot at ${m.end_s.toFixed(0)} s')
    html=html.replace('${m.member_scans} member scans','${m.map_points.toLocaleString()} selected map points from ${m.member_scans} source scans')
html=html.replace('Four-robot overview','Overview')
