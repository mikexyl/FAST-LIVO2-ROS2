...
def validate(path):
    ...
    area=path/'frontend/area_maps'
    if area.exists():
        from s3e_pipeline.area_maps import AreaHistoryAudit
        audit=AreaHistoryAudit()
        manifest=json.loads((area/'manifest.json').read_text())
        if not manifest['complete'] or hashlib.sha256((area/'index.jsonl').read_bytes()).hexdigest()!=manifest['index_sha256']:
            raise ValueError('Invalid area-map manifest')
        rows=[json.loads(line) for line in (area/'index.jsonl').read_text().splitlines()]
        if manifest['submaps']!=len(rows):raise ValueError('Area-map count mismatch')
        for row in rows:
            anchor=updates[row['anchor_scan_id']]
            if row['anchor_sensor_ns']!=anchor['sensor_stamp_ns'] or row['stamp_ns']!=anchor['stamp_ns']:
                raise ValueError('Area snapshot anchor time mismatch')
            T=np.eye(4);T[:3,:3]=Rotation.from_quat(anchor['pose'][3:]).as_matrix();T[:3,3]=anchor['pose'][:3]
            if not np.allclose(T,np.asarray(row['T_world_imu']).reshape(4,4),atol=1e-10):
                raise ValueError('Area snapshot anchor pose mismatch')
            if row['member_scan_ids']:
                if row['begin_ns']!=updates[row['member_scan_ids'][0]]['sensor_stamp_ns'] or row['last_member_ns']!=updates[row['member_scan_ids'][-1]]['sensor_stamp_ns']:
                    raise ValueError('Area point provenance timestamp mismatch')
            payload=area/row['payload']
            if payload.parent!=area or hashlib.sha256(payload.read_bytes()).hexdigest()!=row['sha256']:
                raise ValueError('Area-map payload hash mismatch')
            with np.load(payload,allow_pickle=False) as data:audit.add(row,data)
        result.update(area_snapshots=len(rows),area_retrievable=sum(r['retrievable'] for r in rows),
                      accumulated_area_membership_verified=True,old_in_area_points_retained=True)
