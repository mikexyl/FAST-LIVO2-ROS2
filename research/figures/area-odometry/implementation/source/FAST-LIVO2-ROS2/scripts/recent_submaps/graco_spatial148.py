...
def audit(work, frozen):
    ...
    for robot, trial_path in read(work / 'trials.json').items():
        ...
        assert all(r['schema_version'] == (4 if STRATEGY=='coverage' else 3) and r['strategy'] == STRATEGY for r in source)
        assert all(r['gravity_source'] == 'ellipselio_filter_at_anchor' for r in source)
        profile = yaml.safe_load((SCRIPTS/f'{STRATEGY}_submaps.yaml').read_text())
        if STRATEGY=='spatial':
            ...
            assert all(np.isclose(r['coverage_cell_size_m'],profile['coverage']['cell_size_m']) for r in source)
...
def run(work):
    ...
    try:
        ...
        cfg = yaml.safe_load((SCRIPTS/('coverage_rollout.yaml' if STRATEGY=='coverage' else 'rollout.yaml')).read_text())
        cfg.update(robots=list(FLIGHTS), dataset=str(work/'reference'), output_root=str(work),
                   experiment_name=f'GRACO aerial 05/06/07/08 on 148, {STRATEGY} submaps')
        ...
        for robot in FLIGHTS:
            ...
            p.pop('research', None)
            p['mapping']['submaps'] = yaml.safe_load((SCRIPTS/f'{STRATEGY}_submaps.yaml').read_text())
        ...
        def frontend(item):
            ...
            try:
                ...
                call(['/usr/bin/python3', SCRIPTS/'run_trial.py', robot, '--robot', robot, '--enabled', '--submap-strategy', STRATEGY,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--strategy', choices=['spatial','coverage'], default='spatial')
    args = parser.parse_args()
    STRATEGY=args.strategy
    if STRATEGY=='coverage':
        BASE=ROOT/'.ros2/graco-aerial-coverage148-20260920'
        LIBRARY=BASE/'install/ellipselio/lib/libellipselio_mapping.so'
        LAUNCHER=BASE/'launcher/ellipselio_mapping_mt'
    args.work=args.work or BASE/'full'
    if args.stage == 'run':
