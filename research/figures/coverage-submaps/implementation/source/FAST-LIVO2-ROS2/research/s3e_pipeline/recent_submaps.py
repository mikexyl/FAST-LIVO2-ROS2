...
def prepare(source, output, config, gravity_path=None):
    ...
    if any(row.get('strategy')=='coverage' for row in source_rows):
    ...
    try:
        for row in source_rows:
            ...
            item=dict(row,keyframe_id=key,T_world_body=np.asarray(row['T_world_imu']).reshape(4,4).tolist(),
                      ...
                      submap_points=len(cloud),geometry_preprocessing='causal completed native submap in anchor IMU frame')
            rows.append(item)
