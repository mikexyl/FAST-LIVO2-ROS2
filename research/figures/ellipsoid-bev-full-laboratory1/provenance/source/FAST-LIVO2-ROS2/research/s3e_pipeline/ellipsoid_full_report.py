...
def figures(output):
    ...
    if not any(v['trajectory'] for v in report['branches'].values()):
        from .ellipsoid_unreferenced_report import figures as without_gt
        return without_gt(output)
    truth={}
    for r in ROBOTS:
...
def markdown(output):
    ...
    if not any(v['trajectory'] for v in report['branches'].values()):
        raw_graph=report['branches']['raw'];ellipsoid_graph=report['branches']['ellipsoid']
        index=lines.index('## Trajectory ATE')
        lines[index:index]=[
            f'**Raw BEVs: {len(set(raw_graph["components"].values()))} components, '
            f'{raw_graph["selected_loops"]} selected loops ({raw_graph["selected_inter_robot_loops"]} inter-robot). '
            f'Ellipsoid BEVs: {len(set(ellipsoid_graph["components"].values()))} '
            f'component{"s" if len(set(ellipsoid_graph["components"].values()))!=1 else ""}, '
            f'{ellipsoid_graph["selected_loops"]} selected loops ({ellipsoid_graph["selected_inter_robot_loops"]} inter-robot).**',
            'Full-trajectory accuracy remains unmeasured because timestamped trajectory GT is unavailable.', '']
        lines+=['', '**Full-trajectory ATE is unavailable.** Evo could not produce a trajectory',
        ...
        if Path(report['dataset']).name=='S3E_Laboratory_1':
            lines+=['', 'The supplied Laboratory records describe only endpoints labeled 0 and 1;',
    ...
    lines+=['', 'With timestamped GT, connected trajectories receive one shared SE(3) fit across all robots, with no',
    ...
    if preparation:
        ...
        if (output/'workload.json').exists():
            ...
            if all('raw_nonempty_keyframes' in v for v in workload.values()):
                lines+=['',f'Nonempty descriptor views: **{sum(v["raw_nonempty_keyframes"] for v in workload.values())}/{n} raw**',
                    f'and **{sum(v["ellipsoid_nonempty_keyframes"] for v in workload.values())}/{n} ellipsoid**.',
            ...
            if all('raw_image_extents' in v for v in workload.values()):
                lines+=['', 'The unchanged native ORB detector excludes a 31-pixel image border.',
                    'Images with a dimension at most 62 pixels have no interior beyond that border:',
                    f'**{sum(v["raw_image_extents"]["min_dimension_at_most_62_pixels"] for v in workload.values())}/{n} raw**',
                    f'and **{sum(v["ellipsoid_image_extents"]["min_dimension_at_most_62_pixels"] for v in workload.values())}/{n} ellipsoid**.',
    ...
    if validation:
        ...
        for b,q in validation['loop_quality'].items():
            flagged=str(q['flagged']) if q['gt_checkable'] else 'unavailable'
            lines.append(f'| {b} | {q["gt_checkable"]}/{report["branches"][b]["constraints"]} | {flagged} |')
    ...
    policy_path=output/'provenance/replay-policy.json'
    if policy_path.exists():
        lines+=['', 'Replay policy is retained in [replay-policy.json](provenance/replay-policy.json).',
                f'The configured serial playback rate is **{inputs["config"]["odometry"]["rate"]}x**.',
    ...
    if not (output/'ate.png').exists():lines=[line for line in lines if line!='![Individual ATE](ate.png)']
    if not any(v['trajectory'] for v in report['branches'].values()):
        lines=[line.replace('shows both trajectory sets and GT.','shows trajectories and maps in separate component frames.')
                   .replace('Full poses, constraints, evo ZIPs and shared-alignment evidence, graph residuals',
                            'Full poses, constraints, evo association-status evidence, graph residuals') for line in lines]
    if (output/'maps.png').exists():
        lines+=['', '## Map and BEV inspection', '', '![Corrected component maps](maps.png)', '',
...
def rerun(output):
    ...
    if not any(v['trajectory'] for v in report['branches'].values()):
        from .ellipsoid_unreferenced_report import rerun as without_gt
        return without_gt(output)
