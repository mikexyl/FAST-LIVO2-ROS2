...
def prepare_robot(work,robot,resume=False):
    ...
    cfg=config();export=work/robot/'export';schedule=read_json(work/'schedule.json')['schedule'][robot]
    ...
    inputs=dict(export=file_hash(export/'manifest.json'),schedule=file_hash(work/'schedule.json'))
        ...
        if not cached:
            ...
            try:
                for row,cloud,_ in frames(export):
                    ...
                    if any(s not in expected or s in seen or not 0<=row['stamp_ns']-s<=250000000 for s in requested):
...
def solve(work,resume=False):
    ...
    cfg=config();prepared={r:Path(read_json(work/f'prepared-{r}.json')['path']) for r in ROBOTS}
...
def evaluate(work,output):
    ...
    for branch,graph in graphs.items():
        ...
        report['branches'][branch]=dict(trajectory=evo(dict(poses=dense[branch]),gt,cfg['evaluation'],output/f'evo/{branch}'),
            ...
            optimization_s=graph['optimization_s'])
