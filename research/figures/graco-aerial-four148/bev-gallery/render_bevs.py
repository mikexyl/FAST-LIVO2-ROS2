...
try:
    for robot in cfg['robots']:
        ...
        for key in keys:
            ...
            payload = WORK/f'frontends/{robot}/frontend/submaps'/key['payload']
...
fig.suptitle('GRACO aerial · native ellipsoid BEVs', fontsize=20)
...
<p>253 completed temporal submaps, 10 seconds each with 5 seconds overlap. These are the native grayscale density images reconstructed from the experiment’s saved ellipsoids. Every image reproduces the cached ORB keypoints and descriptors exactly.</p>
...
<h2>Accepted loop pairs</h2><p>Side-by-side views in their original ground-aligned submap frames. The selected pairs survived geometric verification and PCM.</p><select id="pair"></select><div class="panels" id="paired"></div>
...
el('slider').value=8;el('pair').value=2;show();</script></html>'''
