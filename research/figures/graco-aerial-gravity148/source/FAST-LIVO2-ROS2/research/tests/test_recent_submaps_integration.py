...
def test_submap_three_worker_retrieval_is_causal(tmp_path,monkeypatch):
    ...
    def rows(path,values):
        if Path(path).name=='keyframes.jsonl':
            values=[dict(r,submap_id=0,member_scan_ids=list(range(50)),sha256='source',
...
def test_submap_endpoints_native_pcm_cbs_registration(tmp_path,monkeypatch):
    import test_cbs_integration as fixture
    original=fixture.write_jsonl
    def rows(path,values):
        if Path(path).name=='keyframes.jsonl':
            values=[dict(r,submap_id=r['keyframe_id'],available_ns=r['stamp_ns']+200000000,
