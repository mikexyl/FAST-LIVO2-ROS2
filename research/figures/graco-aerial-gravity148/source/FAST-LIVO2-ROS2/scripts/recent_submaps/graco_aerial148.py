#!/usr/bin/env python3
"""Fresh four-flight temporal EllipseLIO / ellipsoid-BEV / PCM / CBS on 148."""
import argparse
...
BASE = ROOT / '.ros2/graco-aerial-four148-20260920'
...
from s3e_pipeline.graco import TOPICS, mapping_config, stage_sensors
...
def verify_conversion(bag, converted):
    """Compare every converted sensor measurement against its ROS1 source."""
    from rosbags.highlevel import AnyReader
    from rosbags.typesys import Stores, get_typestore
    types = get_typestore(Stores.ROS2_HUMBLE)
    meta = yaml.safe_load((converted / 'metadata.yaml').read_text())['rosbag2_bagfile_information']
    assert len(meta['relative_file_paths']) == 1
    count = dict.fromkeys(TOPICS, 0)
    with AnyReader([bag]) as source, sqlite3.connect(
            f'file:{converted / meta["relative_file_paths"][0]}?mode=ro', uri=True) as db:
        selected = [c for c in source.connections if c.topic in TOPICS]
        assert {c.topic for c in selected} == set(TOPICS)
        cursors = {name: iter(db.execute('SELECT timestamp,data FROM messages WHERE topic_id IN '
            '(SELECT id FROM topics WHERE name=?) ORDER BY timestamp,id', (name,))) for name in TOPICS}
        for connection, record_ns, raw in source.messages(connections=selected):
            original = source.deserialize(raw, connection.msgtype)
            converted_ns, cdr = next(cursors[connection.topic])
            current = types.deserialize_cdr(cdr, TOPICS[connection.topic])
            assert record_ns == converted_ns
            assert stamp(original) == stamp(current)
            assert original.header.frame_id == current.header.frame_id
            if connection.topic == '/velodyne/points':
                for name in ('height', 'width', 'is_bigendian', 'point_step', 'row_step', 'is_dense'):
                    assert getattr(original, name) == getattr(current, name), name
                assert [(f.name, f.offset, f.datatype, f.count) for f in original.fields] == [
                    (f.name, f.offset, f.datatype, f.count) for f in current.fields]
                assert np.array_equal(original.data, current.data)
            else:
                for name in ('orientation', 'angular_velocity', 'linear_acceleration'):
                    fields = ('x', 'y', 'z', 'w') if name == 'orientation' else ('x', 'y', 'z')
                    assert all(getattr(getattr(original, name), f) ==
                               getattr(getattr(current, name), f) for f in fields)
                for name in ('orientation_covariance', 'angular_velocity_covariance',
                             'linear_acceleration_covariance'):
                    assert np.array_equal(getattr(original, name), getattr(current, name))
            count[connection.topic] += 1
        for cursor in cursors.values():
            assert next(cursor, None) is None
        expected = {name: sum(c.msgcount for c in selected if c.topic == name) for name in TOPICS}
        assert count == expected
    return dict(counts=count, all_sensor_measurements_equal=True, record_header_point_times_unchanged=True,
                conversion='ROS1 serialization to ROS2 CDR; no filtering within messages or time rebasing',
                ground_truth_used=False)


def stage():
    state = dict(phase='converting_sensors', started_utc=now(), robots={})
    lock = threading.Lock()
    def mark(robot, **fields):
        with lock:
            state['robots'].setdefault(robot, {}).update(fields)
            save(BASE / 'staging-status.json', state)
    def one(item):
        robot, name = item
        bag = Path('/data/graco') / f'{name}.bag'
        converted = BASE / 'converted' / robot
        sensors = BASE / 'inputs' / robot
        mark(robot, phase='converting', source=str(bag))
        call([PYTHON.parent / 'rosbags-convert', '--src', bag, '--dst', converted,
              '--dst-storage', 'sqlite3', '--dst-version', '5', '--include-topic', *TOPICS],
             BASE / f'{robot}-conversion.log', 1800)
        mark(robot, phase='verifying_conversion')
        audit = verify_conversion(bag, converted)
        audit.update(source=str(bag), source_sha256=sha(bag))
        save(BASE / f'{robot}-conversion-audit.json', audit)
        mark(robot, phase='staging_validated_sensors')
        info = stage_sensors(converted, sensors)
        mark(robot, phase='complete', counts=info['counts'], source_sha256=audit['source_sha256'])
    try:
        (BASE / 'converted').mkdir(exist_ok=False)
        (BASE / 'inputs').mkdir(exist_ok=False)
        with ThreadPoolExecutor(max_workers=4) as pool:
            list(pool.map(one, FLIGHTS.items()))
        state.update(phase='complete', finished_utc=now())
        save(BASE / 'staging-status.json', state)
    except Exception as error:
        state.update(phase='failed', error=repr(error), traceback=traceback.format_exc())
        save(BASE / 'staging-status.json', state)
        raise


def export_truth(work):
    """Called only after backend completion; original RTK/INS IMU-frame poses."""
    from rosbags.highlevel import AnyReader
    directory = work / 'reference'
    directory.mkdir(exist_ok=False)
    provenance = {}
    for robot, name in FLIGHTS.items():
        bag = Path('/data/graco') / f'{name}.bag'
        output = directory / f'{robot}_gt.txt'
        previous = None
        count = 0
        with AnyReader([bag]) as reader, output.open('x') as stream:
            connections = [c for c in reader.connections if c.topic == '/gnss/ground_truth']
            assert connections and all(c.msgtype == 'nav_msgs/msg/Odometry' for c in connections)
            for connection, _, data in reader.messages(connections=connections):
                ...
                stream.write(f'{current // 10**9}.{current % 10**9:09d} ' +
                             ' '.join(f'{v:.17g}' for v in values) + '\n')
                count += 1
        provenance[robot] = dict(source=str(bag), source_sha256=read(BASE / f'{robot}-conversion-audit.json')['source_sha256'],
                                topic='/gnss/ground_truth', rows=count, output_sha256=sha(output), evaluation_only=True)
    save(directory / 'source-hashes.json', provenance)
...
def audit(work, frozen):
    ...
    for robot, trial_path in read(work / 'trials.json').items():
        ...
        assert all(r['age_max_s'] <= 10.000001 for r in updates if r['lidar_updated'])
...
def run(work):
    ...
    try:
        ...
        save(work/'calibration-hashes.json', {str(p): sha(p) for p in (BASE/'calibration').glob('*.yaml')})
        ...
        cfg.update(robots=list(FLIGHTS), dataset=str(work/'reference'), output_root=str(work),
                   experiment_name='GRACO aerial 05/06/07/08 on 148, temporal submaps')
        cfg['dpgo'].update(ros_domain_id=168, timeout_s=1800)
        ...
        for robot in FLIGHTS:
            mapping = mapping_config(ROOT, BASE/'calibration', robot, cfg['keyframes'])
        ...
        def frontend(item):
            ...
            try:
                ...
                env = dict(os.environ, ROS_DOMAIN_ID=str(160+2*index))
                mark(robot=robot, status='recording', sensor_duration_s=duration, ros_domain_id=160+2*index)
...
if __name__ == '__main__':
    ...
    parser.add_argument('stage', choices=['stage', 'run', 'backend'])
