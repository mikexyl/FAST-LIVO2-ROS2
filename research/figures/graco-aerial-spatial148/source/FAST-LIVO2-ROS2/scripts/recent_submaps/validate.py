...
def validate(path):
    ...
    if source.exists():
        ...
        for row in rows:
            ...
            if row['schema_version'] not in (1,3):raise ValueError('Unsupported submap snapshot')
            if row['schema_version']==3:
                if row.get('strategy')!='spatial' or row['distance_metric'] not in ('3d','horizontal'):
        ...
        for update in updates:
            ...
            if active['schema_version']==3 and allowed_age>=active['max_age_s']:
