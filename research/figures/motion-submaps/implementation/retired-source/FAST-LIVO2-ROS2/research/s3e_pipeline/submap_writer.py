...
def consume(stream, output):
    ...
    with (output/'index.jsonl').open('x') as index:
        while True:
            ...
            if row['schema_version'] not in (1, 2) or any(r['submap_id'] == row['submap_id'] for r in rows):
                raise ValueError('Invalid or repeated submap')
            if row['schema_version'] == 2:
                if row.get('strategy') != 'motion_overlap' or not set(row['selected_keyframe_scan_ids']).issubset(row['member_scan_ids']):
                    raise ValueError('Invalid motion-submap membership')
            arrays = {}
