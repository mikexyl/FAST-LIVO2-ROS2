...
class RecentSubmaps(unittest.TestCase):
    ...
    def test_motion_snapshot_membership_and_nonretrievable_completion(self):
        row=dict(schema_version=2,strategy='motion_overlap',submap_id=4,geometry_count=2,
                 ellipsoid_count=0,complete=True,retrievable=False,member_scan_ids=[8,9],
                 selected_keyframe_scan_ids=[8])
        def stream():
            header=json.dumps(row).encode()
            return struct.pack('<I',len(header))+header+np.arange(6,dtype='<f4').tobytes()+b'\0'*4
        with tempfile.TemporaryDirectory() as root:
            out=Path(root)/'good';consume(io.BytesIO(stream()),out)
            saved=json.loads((out/'index.jsonl').read_text())
            self.assertTrue(saved['complete']);self.assertFalse(saved['retrievable'])
            row['selected_keyframe_scan_ids']=[7]
            with self.assertRaisesRegex(ValueError,'membership'):
                consume(io.BytesIO(stream()),Path(root)/'invalid')
