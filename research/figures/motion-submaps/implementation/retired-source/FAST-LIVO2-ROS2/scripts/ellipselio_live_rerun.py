...
def main():
    ...
    def analytics(msg):
        ...
        for field in ('active_submap_id','successor_submap_id','handovers','active_features',
                      ...
                      'selected_keyframes','keyframe_overlap','submap_extent_m','successor_support','successor_overlap'):
            if hasattr(msg,field):scalar('/diagnostics/submaps/'+field,getattr(msg,field))
        if getattr(msg,'submap_event',''):
            rr.log('/diagnostics/submaps/events',rr.TextLog(msg.submap_event))
    ...
    def status(complete=False):
        if args.submap_dir and (args.submap_dir/'index.jsonl').exists():
            ...
            for line in (args.submap_dir/'index.jsonl').read_text().splitlines(keepends=True):
                ...
                rr.log(f'/world/archived_submaps/{key}',rr.Points3D(points,colors=[90,115,155],radii=.025))
                rr.log(f'/world/archived_submaps/{key}/metadata',rr.TextDocument(json.dumps(row,indent=2)))
                archived.add(key)
