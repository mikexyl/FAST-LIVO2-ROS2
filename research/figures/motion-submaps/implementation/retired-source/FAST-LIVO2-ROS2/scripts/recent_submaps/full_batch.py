#!/usr/bin/env python3
"""Frozen five-group native-submap → ellipsoid BEV → MapClosures → PCM/CBS batch."""
import argparse
import copy
import hashlib
import json
...
LIBRARY=ROOT/'.ros2/recent-submaps/install/ellipselio/lib/libellipselio_mapping.so'
...
def sources():
    paths=[LIBRARY,ROOT/'.ros2/recent-submaps/launcher/ellipselio_mapping_mt',
           ...
           ROOT/'ellipselio/CMakeLists.txt']
...
def prepare(work,strategy='temporal'):
    import cv2
    from s3e_pipeline.graco import mapping_config
    work.mkdir(parents=True,exist_ok=False)
    template=yaml.safe_load((SCRIPTS/'rollout.yaml').read_text())
    reused={'S3E_Laboratory_4':('Carol','s3e_laboratory_4-carol'),
            'S3E_Campus_Road_1':('Alpha','s3e_campus_road_1-alpha'),
            'S3E_Campus_Road_2':('Alpha','s3e_campus_road_2-alpha'),
            'S3E_Campus_Road_3':('Alpha','s3e_campus_road_3-alpha'),
            'GRACO_ground_01_06':('robot1','graco-ground01-robot1')}
    groups=[]
    for name,version in [('S3E_Laboratory_4','S3Ev1'),('S3E_Campus_Road_1','S3Ev1'),
                         ('S3E_Campus_Road_2','S3Ev2'),('S3E_Campus_Road_3','S3Ev2'),('GRACO_ground_01_06',None)]:
        folder=work/name;folder.mkdir();(folder/'configs').mkdir()
        graco=version is None
        robots=[f'robot{i}' for i in range(1,7)] if graco else ['Alpha','Bob','Carol']
        dataset=ROOT/'.ros2/graco/inputs/reference' if graco else Path('/data/s3e')/version/name
        cfg=copy.deepcopy(template);cfg.update(robots=robots,dataset=str(dataset),experiment_name=name,output_root=str(folder))
        cfg['dpgo']['ros_domain_id']=192
        if graco:
            cfg['dpgo']['timeout_s']=1800
            cfg['evaluation'].update(ground_truth_format='graco_imu_enu',expected_connected_robots=robots,
                trajectory_limitation='GRACO RTK/INS T_Base_Imu positions; GT orientations unused; original timestamps; rigid alignment without scale')
        (folder/'config.yaml').write_text(yaml.safe_dump(cfg,sort_keys=False))
        group=dict(name=name,robots=robots,folder=str(folder),dataset=str(dataset),graco=graco,trials={})
        for robot in robots:
            bag=ROOT/'.ros2/graco/inputs/sensors'/robot if graco else dataset
            if graco:
                config=mapping_config(ROOT,ROOT/'.ros2/graco/inputs/calibration',robot,cfg['keyframes'])
            else:
                config=yaml.safe_load((SCRIPTS/'enabled.yaml').read_text())
                ...
                fs.release()
            params=config['/**']['ros__parameters'];params.pop('research',None);params.pop('use_sim_time',None)
            params['mapping']['submaps']=dict(enabled=True,duration_s=10.,overlap_s=5.)
            if strategy=='motion_overlap':
                params['mapping']['submaps'].update(yaml.safe_load((SCRIPTS/'motion_submaps.yaml').read_text()))
            ...
            reuse=strategy=='temporal' and robot==reused[name][0]
            trial=PREVIOUS/reused[name][1] if reuse else folder/'frontends'/robot
            if reuse:
                old=yaml.safe_load((trial/'frontend/mapping_config.yaml').read_text())['/**']['ros__parameters']
                for key in ('imu','input'):assert old[key]==params[key],(name,robot,key)
                for key,value in params['lidar'].items():
                    if isinstance(value,list):assert np.allclose(value,old['lidar'][key],atol=1e-12,rtol=0)
                    else:assert value==old['lidar'][key],(name,robot,key)
                assert read(trial/'source-hashes.json')[str(LIBRARY.relative_to(ROOT))]==sha(LIBRARY)
            metadata=yaml.safe_load((bag/'metadata.yaml').read_text())['rosbag2_bagfile_information']
            group['trials'][robot]=dict(path=str(trial),reused=reuse,bag=str(bag),config=str(config_path),
                duration_s=metadata['duration']['nanoseconds']/1e9,config_sha256=sha(config_path))
        save(folder/'trials.json',{robot:row['path'] for robot,row in group['trials'].items()})
        groups.append(group)
    ...
    def frontend(task):
        ...
        try:
            ...
            if not item['reused']:
                ...
                call(['/usr/bin/python3',SCRIPTS/'run_trial.py',robot,'--enabled','--robot',robot,
    ...
    for group,row in zip(groups,state['groups']):
        ...
        try:
            ...
            for robot,item in group['trials'].items():
                ...
                call([PYTHON,'-m','s3e_pipeline.recent_submaps','--source',Path(item['path'])/'frontend/submaps',
...
if __name__=='__main__':
    ...
    parser.add_argument('--strategy',choices=['temporal','motion_overlap'],default='temporal')
    args=parser.parse_args()
    if args.stage=='prepare':print(json.dumps(prepare(args.work,args.strategy),indent=2))
