...
def validate(path):
    ...
    if source.exists():
        ...
        for row in rows:
            ...
            if row.get('strategy') == 'motion_overlap':
                keys=row['selected_keyframe_scan_ids']
                if keys!=sorted(set(keys)) or not set(keys).issubset(row['member_scan_ids']):
                    raise ValueError('Invalid selected keyframes')
                if row['last_member_ns']-row['begin_ns']>=round(row['max_age_s']*1e9):
                    raise ValueError('Stale member geometry')
                eligible=(len(keys)>=row['min_retrieval_keyframes'] and row['ellipsoid_count']>=row['min_retrieval_ellipsoids'])
                if row['retrievable'] != (row['complete'] and eligible):
                    raise ValueError('Incorrect motion-submap retrieval admission')
            elif row['retrievable']!=row['complete']:
        ...
        if any(row['age_max_s']>row.get('max_map_age_s',10.)+1e-6 for row in updates if row['lidar_updated']):
