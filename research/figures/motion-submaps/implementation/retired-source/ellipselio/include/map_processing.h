...
class MappingNode : public rclcpp::Node {
 ...
 private:
  ...
  using VoxelSet = std::unordered_set<std::array<int64_t, 3>, VoxelHash>;
  struct MapBuffer {
    ...
    V3D origin = V3D::Zero(), key_position = V3D::Zero();
    Eigen::Quaterniond key_rotation = Eigen::Quaterniond::Identity();
    std::shared_ptr<const VoxelSet> key_voxels;
    double extent_m = 0, key_overlap = 1;
  ...
  std::string submap_strategy_ = "temporal";
  double submap_key_translation_ = 1, submap_key_rotation_ = 10 * M_PI / 180;
  double submap_key_overlap_ = .6, submap_voxel_ = .5, submap_extent_ = 20;
  int submap_keyframes_ = 15, submap_min_support_ = 50, submap_retrieval_keys_ = 3;
  double submap_support_ratio_ = .2;
  int64_t submap_max_age_ns_ = 30000000000LL;
  std::shared_ptr<const VoxelSet> current_voxels_;
  std::string submap_event_;
  void AdvanceMotionSubmaps(int64_t stamp);
  void ObserveSubmapScan();
  void ClearPublishedMap();
  bool SuccessorSupportsScan();
  VoxelSet ScanVoxels() const;
  static double VoxelOverlap(const VoxelSet& current, const VoxelSet& reference);
