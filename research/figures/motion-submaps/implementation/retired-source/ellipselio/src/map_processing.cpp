...
#pragma omp parallel for
  ...
  if (submaps_enabled_ && submap_strategy_ == "motion_overlap")
    current_voxels_ = std::make_shared<const VoxelSet>(ScanVoxels());
  InsertWorldScan();
...
void MappingNode::InsertWorldScan() {
  ...
  if (submaps_enabled_) {
    ...
    if (submap_strategy_ == "motion_overlap") ObserveSubmapScan();
  }
...
#pragma omp parallel for
  for (int i = 0; i < scan_cloud_->size(); i++) {
    ...
    map_->ioctree_.KnnNeighbors(p_world, 1, N_idxs, N_dst, inc_search_rad);
...
MappingNode::MappingNode(
  ...
  submap_key_translation_ = this->declare_parameter<double>("mapping.submaps.keyframe_translation_m", 1.0);
  submap_key_rotation_ = this->declare_parameter<double>("mapping.submaps.keyframe_rotation_deg", 10.0) * M_PI / 180.0;
  submap_key_overlap_ = this->declare_parameter<double>("mapping.submaps.keyframe_overlap", .6);
  submap_voxel_ = this->declare_parameter<double>("mapping.submaps.overlap_voxel_m", .5);
  submap_extent_ = this->declare_parameter<double>("mapping.submaps.extent_m", 20.0);
  submap_keyframes_ = this->declare_parameter<int>("mapping.submaps.max_keyframes", 15);
  const double max_age = this->declare_parameter<double>("mapping.submaps.max_age_s", 30.0);
  submap_min_support_ = this->declare_parameter<int>("mapping.submaps.min_handover_matches", 50);
  submap_support_ratio_ = this->declare_parameter<double>("mapping.submaps.min_handover_overlap", .2);
  submap_retrieval_keys_ = this->declare_parameter<int>("mapping.submaps.min_retrieval_keyframes", 3);
  if (submap_strategy_ != "temporal" && submap_strategy_ != "motion_overlap")
    throw std::invalid_argument("Submap strategy must be temporal or motion_overlap");
  if (!std::isfinite(max_age) || max_age <= 0 || !std::isfinite(submap_extent_) || submap_extent_ <= 0 ||
      !std::isfinite(submap_voxel_) || submap_voxel_ <= 0 || !std::isfinite(submap_key_translation_) || submap_key_translation_ <= 0 ||
      !std::isfinite(submap_key_rotation_) || submap_key_rotation_ <= 0 ||
      !std::isfinite(submap_key_overlap_) || submap_key_overlap_ <= 0 || submap_key_overlap_ >= 1 ||
      !std::isfinite(submap_support_ratio_) || submap_support_ratio_ <= 0 || submap_support_ratio_ > 1 ||
      submap_keyframes_ < 4 || submap_min_support_ < 2 || submap_retrieval_keys_ < 1 || submap_retrieval_keys_ > submap_keyframes_)
    throw std::invalid_argument("Invalid motion/overlap submap settings");
...
#ifdef ELLIPSELIO_RESEARCH_EXPORT
  if (submap_export_ || diagnostics_.is_open()) {
...
void MappingNode::TimerCallback() {
  ...
  if (SyncPackages()) {
    ...
    analytics_msg_.selected_keyframes = map_->keyframes.size();
    analytics_msg_.keyframe_overlap = map_->key_overlap;
    analytics_msg_.submap_extent_m = map_->extent_m;
    ...
    if (diagnostics_.is_open()) {
      ...
      nlohmann::json row = {{"scan_id", map_counter_}, {"sensor_stamp_ns", scan_end_time_.nanoseconds()},
        ...
        {"submap_keyframe", !map_->keyframes.empty() && map_->keyframes.back() == map_counter_}, {"selected_keyframes", map_->keyframes.size()},
        {"keyframe_overlap", map_->key_overlap}, {"submap_extent_m", map_->extent_m},
        {"successor_support", analytics_msg_.successor_support},
        {"successor_overlap", analytics_msg_.successor_overlap},
        {"max_map_age_s", (submap_strategy_ == "motion_overlap" ? submap_max_age_ns_ : submap_duration_ns_)*1e-9},
