...
def normalize_exported_basis(basis, tolerance=1e-3):
    """Remove bounded native float tensor roundoff; reject materially invalid bases.

    Preserve reflection parity: ellipsoid axes need an orthogonal basis, not SO(3).
    The original bitwise geometry is retained in the source delta exports.
    """
    basis = np.asarray(basis, dtype=float)
    if basis.ndim != 3 or basis.shape[1:] != (3, 3) or not np.isfinite(basis).all():
        raise ValueError('Invalid exported ellipsoid basis')
    if not 0 < tolerance <= 1e-3:
        raise ValueError('Unsupported ellipsoid roundoff tolerance')
    error = np.max(np.abs(basis.transpose(0, 2, 1) @ basis - np.eye(3)), axis=(1, 2))
    if np.any(error > tolerance):
        raise ValueError(f'Ellipsoid basis defect exceeds roundoff bound: {error.max()}')
    u, _, vt = np.linalg.svd(basis)
    normalized = u @ vt
    return normalized, dict(method='nearest orthogonal basis via SVD; reflection parity preserved',
        tolerance=tolerance, max_gram_error=float(error.max()) if len(error) else 0.,
        count=len(basis), max_basis_change=float(np.max(np.abs(normalized-basis))) if len(error) else 0.)
