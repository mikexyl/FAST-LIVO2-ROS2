...
`build.sh` and `env.sh` record the machine's native dependency prefixes; adjust
those paths when moving the setup. Only `rtabmap_msgs` is required from RTAB-Map
...
* A read-only processing-stamp acknowledgement enables bounded offline replay:
  at most five unacknowledged scans and five unpublished descriptors per robot.
  This avoids losses while the upstream single-threaded callbacks register clouds.
  It does not enter the native selector, retrieval, registration, or optimizer.
...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .ros2/swarm/venv/bin/python -m pytest \
  Swarm-SLAM/src/cslam/tests Swarm-SLAM/s3e/test_registration.py -q'
```

Upstream unit tests plus the transform checks passed (27 tests). The 30-second
