...
def figures(output):
    ...
    gt_available=any(b['trajectory'] for b in models.values())
    if gt_available:
...
def markdown(output):
    ...
    if not run['success']:lines += [f'Reason: `{run.get("error","not available")}`. Partial native outputs are retained; no completed-run ATE is invented.','']
    ...
    if not any(v['trajectory'] for v in models.values()):
...
def rerun(output):
    ...
    rr.send_blueprint(rrb.Blueprint(rrb.Vertical(rrb.Horizontal(*columns),rrb.TextDocumentView(origin='/report'),row_shares=[.8,.2]),collapse_panels=True))
