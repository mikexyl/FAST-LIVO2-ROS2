...
def figures(output):
    ...
    for b,v in models.items():
        ...
        data=rows(source)
        for robot in ROBOTS:
            if robot not in v['components']:continue
...
def markdown(output):
    ...
    if not run['success']:lines += [f'Reason: `{run.get("error","not available")}`. Input delivery and native optimized coverage are checked separately below.','']
    ...
    elif swarm.get('trajectory_evaluable'):
        lines += ['Actual trajectory error is evaluated for each robot with complete native optimized coverage.',
                  'The full controlled comparison is invalid if input selection or any robot output is incomplete; missing robots are excluded.','']
    else:
    ...
    if run.get('total_wall_s') is not None:
        lines += [f'Swarm wall time: **{run["total_wall_s"]:.2f} s**, including **{fmt(run.get("sensor_replay_wall_s"))} s** of completed sensor replay and',
