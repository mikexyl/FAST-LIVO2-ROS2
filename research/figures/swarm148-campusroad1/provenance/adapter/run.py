...
class Observer:
    def __init__(self,node,output):
        ...
        self.scans={r:ScanTracker() for r in range(3)}
    ...
    def ack(self,robot,msg):
        self.scans[robot].acknowledge(msg.data)
...
def main():
    ...
    p.add_argument('--drain-timeout-s',type=float,default=120.)
    ...
    if args.rate<=0 or args.max_inflight<1 or args.drain_timeout_s<=0 or args.max_wall_s<=0:
        raise ValueError('Replay rate and limits must be positive')
    ...
    params['frontend']['reliable_input']=True
    ...
    write_json(out/'inputs.json',dict(frontend=str(work/'frontend'),rate=args.rate,duration=args.duration,
        ...
        replay_backpressure=f'At most {args.max_inflight} exactly tracked unacknowledged scans and five unpublished descriptors per robot; reliable QoS; causal timestamp order.',
        drain_timeout_s=args.drain_timeout_s,max_wall_s=args.max_wall_s,
        scripts={str(f.relative_to(ROOT)):file_hash(f) for f in [Path(__file__),Path(__file__).with_name('replay_tracking.py'),Path(__file__).with_name('frontend_quality.py'),ROOT/'Swarm-SLAM/src/cslam/cslam/lidar_pr/icp_utils.py',ROOT/'Swarm-SLAM/src/cslam/cslam/lidar_handler_node.py']}))
    ...
    resource_samples={}
    def check():
        if time.monotonic()-observer.started>args.max_wall_s:
            raise TimeoutError('Run exceeded explicit wall-time budget')
    ...
    def progress(phase,elapsed=None):
        write_json(out/'progress.json',dict(phase=phase,wall_s=time.monotonic()-observer.started,
            input_elapsed_s=elapsed,published_scans=published,
            processed_scans=[len(observer.scans[r].acked) for r in range(3)],
            pending_stamps_ns=[sorted(observer.scans[r].pending) for r in range(3)],
            expected_keyframes=expected,received_keyframes=[len(observer.keys[r]) for r in range(3)],
            received_descriptors=[len(observer.descriptors[r]) for r in range(3)],
            optimized_poses={str(r):len(v['estimates']) for r,v in observer.optimized.items()}))
    try:
        quality={robot:check_export(work/'frontend'/('Alpha' if args.smoke_duplicate_alpha else robot)/'export') for robot in ROBOTS}
        write_json(out/'frontend-quality.json',quality)
        if not all(q['passed'] for q in quality.values()):
            raise ValueError('Frontend odometry failed the ground-truth-free motion sanity check; native Swarm was not started')
        ...
        while not (all(pc.get_subscription_count()>0 and od.get_subscription_count()>0 for pc,od in pubs)
                   and all(node.count_subscribers(f'/r{r}/cslam/keyframe_data')>=1
                           and node.count_subscribers(f'/r{r}/cslam/keyframe_odom')>=2
                           and observer.counts[f'/r{r}/cslam/heartbeat']>0 for r in range(3))):
        ...
        while queue:
            ...
            while True:
                if len(observer.scans[r].pending)<args.max_inflight and len(observer.keys[r])-len(observer.descriptors[r])<5:break
            ...
            observer.scans[r].publish(stamp)
        ...
        while not all(not observer.scans[r].pending and len(observer.keys[r])==expected[r] and len(observer.descriptors[r])==expected[r] for r in range(3)):
            if time.monotonic()>deadline:raise TimeoutError('Native keyframes/descriptors did not drain within bounded wait')
            spin(.1)
            if time.monotonic()>=next_report:
                progress('draining');next_report=time.monotonic()+10
        summary['input_replay_complete']=True
        progress('final_optimization')
        ...
        summary['processed_scans']=[len(observer.scans[r].acked) for r in range(3)]
        summary['pending_stamps_ns']=[sorted(observer.scans[r].pending) for r in range(3)]
        summary['total_wall_s']=time.monotonic()-observer.started
        summary['backpressure_s']=backpressure_s
        progress('complete' if summary['success'] else 'failed')
        ...
        observer.events.close();node.destroy_node()
