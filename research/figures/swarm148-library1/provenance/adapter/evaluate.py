...
def evaluate(work,output):
    ...
    if (output/'ours/report.json').exists():
        ...
        comparison_status=({'failure':read_json(work/'comparison-failure.json')}
                           if (work/'comparison-failure.json').exists()
                           else {'not_run':'No fresh paired MapClosures run requested for this remote Swarm test.'})
        ours=dict(**comparison_status,branches={b:dict(trajectory={},components={},available=False)
    ...
    covered_robots=[r for i,r in enumerate(ROBOTS)
                    if str(i) in results and keys[r] and len(results[str(i)]['estimates'])==len(keys[r])]
    report['swarm'].update(keyframe_input_audit=input_audit,
        ...
        fully_covered_robots=covered_robots,
    ...
    # Missing replay scans invalidate a controlled input comparison, not evo's
    # measurement of the actual recorded native trajectory. Keep both statuses.
    report['swarm']['trajectory_evaluable']=bool(covered_robots)
    if covered_robots:
        dense=[];optimized=[]
        for index,robot in enumerate(ROBOTS):
            if robot not in covered_robots:continue
        ...
        report['swarm']['trajectory_scope']='Actual recorded native optimized trajectories only for robots with full per-robot coverage, densely corrected using associated odometry; missing robots are excluded and input-comparison validity is reported separately.'
