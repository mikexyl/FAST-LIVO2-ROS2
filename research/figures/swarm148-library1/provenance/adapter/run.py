...
def main():
    ...
    write_json(out/'inputs.json',dict(frontend=str(work/'frontend'),rate=args.rate,duration=args.duration,
        ...
        scripts={str(f.relative_to(ROOT)):file_hash(f) for f in [Path(__file__),Path(__file__).with_name('replay_tracking.py'),ROOT/'Swarm-SLAM/src/cslam/cslam/lidar_pr/icp_utils.py',ROOT/'Swarm-SLAM/src/cslam/cslam/lidar_handler_node.py']}))
