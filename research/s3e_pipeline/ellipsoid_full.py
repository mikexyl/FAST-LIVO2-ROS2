...
def prepare_robot(work,robot,resume=False):
    ...
    if robot not in cfg['robots']:raise ValueError('Robot is absent from the experiment configuration')
    branches=cfg.get('descriptor_branches',['raw','ellipsoid'])
    if not branches or set(branches)-{'raw','ellipsoid'}:raise ValueError('Invalid descriptor branches')
        ...
        if not cached:
            ...
            try:
                for row,cloud,_ in frames(export):
                    ...
                    rows.append(item)
                    raw_started=time.monotonic();rf=engine().describe(raw.astype(float)) if 'raw' in branches else None;raw_s=time.monotonic()-raw_started
                    ...
                    if len(g['centers']):
                        ...
                        if cfg.get('ellipsoid_basis_roundoff_tolerance') is not None:
                            from .ellipsoid_bev import normalize_exported_basis
                            basis,basis_diagnostic=normalize_exported_basis(basis,cfg['ellipsoid_basis_roundoff_tolerance'])
                        started_render=time.monotonic();ellipsoid,sampling=sampler.render(centers,g['axes'],basis)
                        sampling['basis_roundoff']=basis_diagnostic
                    ...
                    timing=dict(keyframe_id=key,stamp_ns=row['stamp_ns'],ellipsoids=len(g['centers']),
                        raw_orb=len(rf['xy']) if rf is not None else 0,ellipsoid_orb=len(ef['xy']),raw_descriptor_s=raw_s,
                    ...
                    if time.monotonic()>=next_report:
                        ...
                        print(f'{robot}: {len(rows)}/{len(schedule)} keyframes, '+
                              ...
                              f'ORB raw/ellipsoid={timing["raw_orb"]}/{len(ef["xy"])}',flush=True)
...
def evaluate(work,output):
    ...
    if not any(v['trajectory'] for v in report['branches'].values()):
        # Unavailable GT is an explicit evaluation outcome. Preserve maps in
        # each graph component's own frame; never fabricate a GT alignment.
        from .ellipsoid_full_maps import build_maps,retain_bevs
        build_maps(work,output);retain_bevs(work,output)
...
def main():
    ...
    p.add_argument('--work',type=Path,required=True);p.add_argument('--robot')
