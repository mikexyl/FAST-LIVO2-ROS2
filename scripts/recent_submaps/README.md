...
For MapClosures snapshots containing **all accumulated map points inside a
horizontal region around the drone**, see [AREA-MAPS.md](AREA-MAPS.md).
`--area-maps` enables that separate persistent map; odometry keeps its recent-map
policy. Use `area_rollout.yaml` for its backend. Snapshot cadence never limits
the age of points selected into an area map.

Temporal submapping remains the default. The new [coverage strategy](COVERAGE-SUBMAPS.md)
uses observed occupied area and measured overlap; select it with
`--submap-strategy coverage` and pair it with `coverage_rollout.yaml`. An experimental `spatial` strategy is
...
The isolated four-flight runner `graco_spatial148.py run` uses the retained,
validated aerial-05–08 sensor bags and calibration, four concurrent fresh 1×
captures, and gravity-horizontal ellipsoid BEV → MapClosures → PCM/CBS. It expects
the dedicated spatial build/environment on 148 and refuses to overwrite its
output directory. `render_spatial_bevs.py --work WORK --output GALLERY` verifies
and displays the exact cached native features. See the
[completed experiment report](../../research/RESULTS-GRACO-AERIAL-SPATIAL148.md).
